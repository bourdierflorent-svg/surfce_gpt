...
import { createAppointmentRequestSchema } from "@/features/opportunities/schemas";
import { createOpportunityAppointment } from "@/features/opportunities/server/service";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json(await createOpportunityAppointment(context, id, parsed.data), {
      status: 201,
    });
  } catch (error) {
